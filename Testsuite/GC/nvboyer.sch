(error "Yadda yadda")
*** NOT YET FINISHED -- like nboyer, but using vectors rather than pairs
*** to be more comparable in space usage to Java.
...
; several bugs that are corrected here.  These bugs are discussed by Henry 
; Baker, "The Boyer Benchmark Meets Linear Logic", ACM SIGPLAN Lisp Pointers 
; 6(4), October-December 1993, pages 3-10.  The fixed bugs are:
;
...
(let ()
  ...
  (define (vcons a b) (vector a b))
  (define (vcar x) (vector-ref x 0))
  (define (vcdr x) (vector-ref x 1))
  ...
  (define (translate-term term)
    (cond ((not (pair? term))
           term)
          (else (vcons (symbol->symbol-record (car term))
		       (translate-args (cdr term))))))
  
  (define (translate-args lst)
    (cond ((null? lst)
           '())
          (else (vcons (translate-term (car lst))
  ...
  (define (untranslate-term term)
    (cond ((not (vpair? term))
           term)
          (else (cons (get-name (vcar term))
		      (map untranslate-term (vcdr term))))))
    ...
    (cond ((not (vpair? term))
           (let ((temp-temp (assq term alist))) ;########## was global
             (if temp-temp
                 (cdr temp-temp)
                 term)))
          (else (vcons (vcar term)
		       (apply-subst-lst alist (vcdr term))))))
  
  (define (apply-subst-lst alist lst)
    (cond ((null? lst)
           '())
          (else (vcons (apply-subst alist (vcar lst))
		       (apply-subst-lst alist (vcdr lst))))))
