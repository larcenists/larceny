...
(define-syntax mini-meta
  (syntax-rules (or and begin)
    ((_ (or x ...))
     (lambda (forms)
       (gen-parse forms () (and (or x ...)))))
    
    ((_ (and x ...))
     (lambda (forms)
       (gen-parse forms () (and x ...))))
...
(define-syntax gen-parse
  (syntax-rules (and lambda or and begin quote ?* ?)
    ...
    ((_ forms () (and (lambda () bdy ...)))
     (and (null? forms)
	  (begin bdy ...)))
    
    ((_ forms (m1) (and (lambda (a1) bdy ...)))
     (and (null? forms)
	  (let ((a1 m1))
	    (begin bdy ...))))
    
    ((_ forms (m2 m1) (and (lambda (a1 a2) bdy ...)))
     (and (null? forms)
	  (let ((a1 m1) (a2 m2))
	    (begin bdy ...))))

    ((_ forms (m3 m2 m1) (and (lambda (a1 a2 a3) bdy ...)))
     (and (null? forms)
	  (let ((a1 m1) (a2 m2) (a3 m3))
	    (begin bdy ...))))
  ...
  (define (handle-text-symbol type target value . maybe-size)
    ...
    (let* ((pnt   (push-stack-push->patcher
	   ...
	   (a-reloc (make-sassy-reloc
		     ...
		     'text offs type #f value current-byte-size
	 ...
	 (fix-disp
	  (lambda () (if (or disp symb)
			 (cond ((and (s-byte disp) (not symb))
				...
				(set! disp (if disp `(dword ,disp)
					       '(dword 0))))
			       (else (bad-mem))))))
	 (skale (lambda (x)
		  (cond ((equal? x 1) x)
			((equal? x 2) (set! sk 64))
			((equal? x 4) (set! sk 128))
			((equal? x 8) (set! sk 192)))))

	 (mem-i
	  (lambda (u)
	    (cond ((r32 u) =>
		   (lambda (x)
		     (cond ((not r/m) (set! r/m x))
			   ((not ix) (if (not (= x 4))
					 (set! ix (* 8 x))
					 (begin (set! ix (* 8 r/m))
						(set! r/m 4))))
		   ...
		   (lambda (x)
			 ...
			 (if (symbol? x)
			     ...
			     (begin (set! type (cadr x))
				    ...
				    (if (not disp)
					(set! disp (cadddr x))
					(set! disp (+ disp (cadddr x))))))
			 (bad-mem))))

		  ((integer? u) (if (not disp)
				    (set! disp u)
				    (set! disp (+ u disp))))

		  ((and (pair? u) (eq? '* (car u)))
		   (or (and (skale (cadr u))
			    (let ((y (r32-not-esp (caddr u))))
			      (and y (set! ix (* 8 y)))))
		       (and (skale (caddr u))
			    (let ((y (r32-not-esp (cadr u))))
			      (and y (set! ix (* 8 y)))))
		       (bad-mem)))
		  (else (bad-mem))))))
    ...
    (let ((rep-able
	  ...
	  ((just-c     (lambda (sizer opcode)
	   ...
	   (i16-i32    (lambda (sizer opcode imm1 imm2)
			 ...
			 (handle-imm 4 imm1)
			 (just-c sizer opcode)))
	   (lof-imms (lambda (sizer itms)
		       (for-each
			(lambda (itm)
			  (cond ((string? itm)
				 (let ((end-fill
					(- sizer
					   (modulo (string-length itm)
						   sizer))))
				   (push-stack-push (t-text t)
						    (make-list end-fill 0))
				   (push-stack-push (t-text t)
						    (map char->integer
							 (string->list itm)))))
				(else (handle-imm sizer itm))))
			(reverse itms)))))
	(let
	    (
	     (gen-non
		...
		(mini-meta
		 (or (and r32 (lambda (x) (just-r 4 partial-code x)))
		     ...
		     (and (or r8 em8) (lambda (x)
					(r/m 1 #xfe x reg-field)))))))
	     (gen-plier
	      (lambda (reg-field)
		(mini-meta
		...
		(mini-meta
	     ...
	     (gen-r/rm
	      (lambda (opcodes)
		(mini-meta
		 (or (and r32 (or r32 m32) (lambda (x y)
					     (r-r/m 4 opcodes x y)))
		...
		(mini-meta
		 (and pred (lambda (x)
			     (and ((hash-table-ref the-opcodes (car x))
				   (cdr x))
				  (outc code)))))))
	     (raw-bytes? (lambda (lst p)
			   (let iter ((lst lst)
				      (res '()))
	   ...
	   `(; raw data in the text section
	     (bytes  . ,(lambda (lst)
			  (cond ((raw-bytes? lst i8) =>
				 (lambda (good) (lof-imms 1 good)))
				(else #f))))
	     (words  . ,(lambda (lst)
			  (cond ((raw-bytes? lst i16) =>
				 (lambda (good) (lof-imms 2 good)))
				(else #f))))
	     
	     (dwords  . ,(lambda (lst)
			  (cond ((raw-bytes? lst i32) =>
				 (lambda (good) (lof-imms 4 good)))
				(else #f))))
