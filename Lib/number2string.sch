...
;
; $Id: number2string.sch,v 1.6 1992/06/13 03:52:22 remy Exp $
;
;                                       16 April 1990
;
;
	     ...
	     (error "number->string, compnums not implemented yet"))
            ((= (float-exponent x) flonum:maxexponent)
             (cond ((not (= x x))
                   ...
                   ((positive? x)
                    (string-copy "#<INFINITY>"))
                   (else (string-copy "-#<INFINITY>"))))
            ((= radix 10)
             (string-append (if (negative? x) "-" "")
                            (dragon (float-significand x)
                                    (float-exponent x))))
            ((zero? x) (string-copy "#i0"))
            (else (let* ((p (abs (inexact->exact (numerator x))))
