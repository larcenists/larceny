; UNDER CONSTRUCTION!
; This will be a faster reader.  The code is based on the existing one.
; Lib/reader.sch
; Larceny -- Scheme reader
;
; $Id: reader.sch,v 1.1 1997/09/17 15:06:05 lth Exp lth $
;
...
(define read)
(define readtable-ref)
(define readtable-set!)

(define (make-reader character-syntax-table)

  ; Character class predicates.

  ; Some of these are macros in the MacScheme version and come from 
  ; "reader0.sch", but as procedures, they have to be defined inside 
  ; this procedure.

  (define (whitespace? x)
    (not (zero? (logand 1 (bytevector-ref character-syntax-table
					  (char->integer x))))))

  (define (separator? x)
    (not (zero? (logand 2 (bytevector-ref character-syntax-table
					  (char->integer x))))))

  (define (digit? x)
    (not (zero? (logand 4 (bytevector-ref character-syntax-table
					  (char->integer x))))))

  (define (alphabetic? x)
    (not (zero? (logand 8 (bytevector-ref character-syntax-table
					  (char->integer x))))))
  ...
  (define (tyipeek p)
    (if (valid-io-pointer? p)
	(io-buffer-ref p)
	(eof-object)))

  (define (tyinext p)
    (if (advance-io-pointer p)
	(io-buffer-ref p)
	(eof-object)))

  (define (tyi p)
    (if (valid-io-pointer? p)
	(let ((c (io-buffer-ref p)))
	  (advance-io-pointer p)
	  c)
	(eof-object)))
...
;                      (read-symbol (tyipeek p) p (cons c l))))))
  ...
  (let ((character-syntax-table (make-syntax-table)))
...
(define (make-syntax-table)
  ...
  ; Each character's entry is an integer x = a0 + 2*a1 + 4*a2 + 8*a3 where
  ;
  ...
  ;     a3 is 1 iff the character is alphabetic.
  ...
  (do ((c 0 (+ 1 c)))
      ((> c 31) '())
    (bytevector-set! character-syntax-table
		     c
  ...
  (do ((c (char->integer #\a) (+ c 1)))
      ((> c (char->integer #\z)))
    (bytevector-set!
     character-syntax-table
     x
     (logior 8 (bytevector-ref character-syntax-table c))))
