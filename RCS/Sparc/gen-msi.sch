...
(define (emit-global->register! as offset r)
  ...
  (define (emit-undef-check! as r)
	...
	(let ((l1 (new-label)))
	  ...
	  (emit! as `(,$i.jmpli ,$r.millicode ,$m.generic-exception ,$r.o7))
...
;
; The exception handling code assumes that an args= instruction is always the
; first instruction in the current procedure. The compiler may not violate
; this invariant. DOCUMENTME!
...
; millicode to gather arguments into a list. The exception code makes the
; same assumptions as for "args=", above.
;
; Perhaps the return address should have been adjusted here.
...
; which is in the topmost frame on the stack.
;
...
(define (emit-invoke! as n)
  (let ((l0 (new-label))
    ...
    (emit! as `(,$i.jmpli ,$r.millicode ,$m.generic-exception ,$r.g0))
