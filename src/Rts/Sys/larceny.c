...
static char *wizardhelptext[] = {
  ...
  "  -oracle",
  "     (RROF) Turns on remembered set updating via an untimed oracle.  Simulates",
  "     how the regional garbage collector might perform if we had a zero cost",
  "     remembered set representation, thus providing an estimate how how much",
  "     improvement we might get from changes to the remembered set representation",
  "  -refinement d",
  "     (RROF) Turns on periodic mark pass during collection to refine remembered",
  "     sets.  The parameter d roughly represents the number of words of allocation",
  "     allowed for each word of marking.",
  "  -mark_period n",
  "     (RROF) Turns on periodic mark pass during collection to refine remembered",
  "     sets.  The mark pass will be invoked every n nursery evacuations.",
#if ROF_COLLECTOR
  "  -steps n",
