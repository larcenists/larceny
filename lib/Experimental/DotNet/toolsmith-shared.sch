  ...
  (let* ((lines-rep    'vector)
    ...
    (msg-handler
     ...
     ((set-selection! start-pos-incl end-pos-excl)
      ...
      ((wnd 'update)))
     ((on-keydown sym mods)
      (set! event-log (cons 'on-keydown event-log))
      '(begin (write `(keydown ,((wnd 'title)) ,sym ,mods)) (newline))
      )
     ((on-keyup   sym mods)
      (define num-tries 0)
      (set! event-log (cons 'on-keyup event-log))
      ...
      ((wnd 'update)))
     ((on-resize)
      (set! event-log (cons 'on-resize event-log))
      (set! width  ((wnd 'width)))
      (set! height ((wnd 'height))))
     ((on-keypress char) 
      (set! event-log (cons 'on-keypress event-log))
     ...
     ((on-hscroll new-int event-type)
      (set! event-log (cons 'on-hscroll event-log))
      (begin (write `(on-hscroll wnd ,new-int ,event-type))
             (newline)))
     ((on-vscroll new-int event-type)
      (set! event-log (cons 'on-vscroll event-log))
      ...
      ((wnd 'update)))
     ((on-mousedown mx my)
      (set! event-log (cons 'on-mousedown event-log))
      ...
      ((wnd 'update)))
     ((on-mouseup mx my)
      (set! event-log (cons 'on-mouseup event-log))
      (begin (display event-log)
             (newline)
             (set! event-log '()))
      ...
      ((wnd 'update)))
     ((on-mousedrag mx my)
      (set! event-log (cons 'on-mousedrag event-log))
      (cond (mouse-drag
             (set-car! mouse-drag mx)
             (set-cdr! mouse-drag my)))
      ((wnd 'update)))
     ((on-paint g rx ry rw rh)
      (set! event-log (cons 'on-paint event-log))
