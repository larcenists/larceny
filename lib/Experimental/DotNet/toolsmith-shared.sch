...
(define (make-code-editor-agent wnd width height)
  ...
  (let* ((lines-rep    'vector)
    ...
    ;; the text structure.
    (define (pixel-coord->text-coord mx my)
    ...
    (msg-handler
     ...
     ((on-vscroll new-int event-type)
      ...
      ((wnd 'update)))
     ((on-mousedown mx my)
      (let-values (((row col) (pixel-coord->text-coord mx my)))
        (begin (display `(mousedown (mx: ,mx) (my: ,my) (row: ,row) (col: ,col)))
               (newline))
        (let* ((newval (text-coord->text-pos row col)))
          (set! mouse-down-pos newval)
          (set! selection-start-pos mouse-down-pos)
          (set! selection-finis-pos mouse-down-pos)))
      ((wnd 'update)))
     ((on-mouseup mx my)
      (let-values (((row col) (pixel-coord->text-coord mx my)))
        (begin (display `(mouseup (mx: ,mx) (my: ,my) (row: ,row) (col: ,col)))
               (newline))
        (let* ((newval (text-coord->text-pos row col)))
          (set! selection-start-pos (min mouse-down-pos newval))
          (set! selection-finis-pos (max mouse-down-pos newval))))
      ((wnd 'update)))
     ((on-mousedrag mx my)
      (let-values (((row col) (pixel-coord->text-coord mx my)))
        (begin (display `(mousedrag (mx: ,mx) (my: ,my) (row: ,row) (col: ,col)))
               (newline))
        (let* ((newval (text-coord->text-pos row col)))
          (set! selection-start-pos (min mouse-down-pos newval))
          (set! selection-finis-pos (max mouse-down-pos newval))))
