...
;;; BUG: char-set-sre? is apparently intended to return true if and only if
;;; its argument is a <cset-sre>.  It fails to do that.
;;; There are several problems:
;;;
;;; 1. If (car sre) is &, sre is not a <cset-sre>.  (I'm going to assume
;;;    SRFI 115 failed to mention & is supposed to be a synonym for and.)
;;; 2. or and - difference ~ complement w/case w/nocase w/ascii w/unicode
;;;    should be treated the same here.
;;; 3. SRFI 115 fails to list |\|| as synonym for or in the productions for
;;;    <cset-sre>.  (I'm going to assume that's a mistake in SRFI 115.)
;;; 4. SRFI 115 says a <cset-sre> can be a <char-set> (embedded SRFI 14
;;;    char set), but there is no external representation for a SRFI 14
;;;    character set.  (I'm going to assume that's a mistake in SRFI 115.)
     ...
     ((pair? sre)
      (if (string? (car sre))
          ...
          (case (car sre)
            ...
            ;; BUG: these next four cases were left out of the
