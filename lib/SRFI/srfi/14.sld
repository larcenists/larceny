...
; a little over 139264 bytes of memory.
;
...
;;; FIXME: Twobit would probably propagate the constant without
;;; having to use define-syntax here.

(define-syntax %char-set:minsize
  (syntax-rules ()
   ((_) 128)))

(define-syntax %char-set:minsize-in-bytes
  (syntax-rules ()
   ((_) 16)))
...
(define %excluded:max #xdfff)    ; (inclusive) is reserved for surrogate pairs
...
(define char-set:full
  (ucs-range->char-set 0 %unicode:limit))
