; This is an R6RS version of the nboyer benchmark.
; It reads the benchmark size from (current-input-port).
...
(library (local run-benchmark)
  ...
  ; This definition of time may be replaced by a more useful
...
(library (local nboyer)

  (export setup-boyer test-boyer)
...
(import (rnrs) (local nboyer) (local run-benchmark))
...
(define n (read))
(display "Running nboyer on problem size: ")
(write n)
(newline)
(nboyer-benchmark n)
