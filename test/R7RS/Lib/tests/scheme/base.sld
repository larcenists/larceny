...
#;     (test (port-profile (close-port (current-input-port)))
             '(#t #f #t #f #t #f #f))
#;     (test (port-profile (close-port (current-output-port)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-port (current-error-port)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-port (open-input-string "whatever")))
             '(#t #f #t #f #t #f #f))
       (test (port-profile (close-port (open-output-string)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-port (open-input-bytevector '#u8(0 0 7))))
             '(#t #f #f #t #t #f #f))
       (test (port-profile (close-port (open-output-bytevector)))
             '(#f #t #f #t #t #f #f))

       ;; Closing (current-input-port) twice may not work.

#;     (test (port-profile (close-input-port (current-input-port)))
             '(#t #f #t #f #t #f #f))
       (test (port-profile (close-input-port (open-input-string "whatever")))
             '(#t #f #t #f #t #f #f))
       (test (port-profile (close-input-port (open-input-bytevector '#u8())))
             '(#t #f #f #t #t #f #f))

#;     (test (port-profile (close-output-port (current-output-port)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-output-port (current-error-port)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-output-port (open-output-string)))
             '(#f #t #t #f #t #f #f))
       (test (port-profile (close-output-port (open-output-bytevector)))
             '(#f #t #f #t #t #f #f)))
     ...
     (call-with-port
      ...
      (lambda (adams)
        ...
        (test (read-string 25 adams) "f the Western Spiral arm ")
     ...
     (parameterize
      ...
      (test (eof-object? (eof-object)) #t)
      (test (char-ready?) #t)
      (test (read-string 25) "f the Western Spiral arm ")
     ...
     (call-with-port
      ...
      (lambda (zelazny)
        ...
        (test (eof-object? (read-bytevector 0 zelazny)) #t)
        (test (eof-object? (read-bytevector 1000 zelazny)) #t)
        (test (let ((bv (make-bytevector 5 0)))
                (test (read-bytevector! bv zelazny) 0)
                bv)
              '#u8(0 0 0 0 0))))
...
#;
     (parameterize
      ...
      (test (eof-object? (read-bytevector 0)) #t)
      (test (eof-object? (read-bytevector 1000)) #t)
      (test (let ((bv (make-bytevector 5 0)))
              (test (read-bytevector! bv) 0)
              bv)
            '#u8(0 0 0 0 0)))
