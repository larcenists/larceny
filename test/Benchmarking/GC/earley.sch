...
(define (earley-benchmark . args)
  (let* ((k (if (null? args) 9 (car args)))
         (iters (if (or (null? args) (null? (cdr args))) 1 (cadr args)))
         (name-prefix (if (= iters 1) "" (number->string iters)))
         (name-suffix (string-append ":" (number->string k))))
    (run-benchmark
     (string-append name-prefix "earley" name-suffix)
     iters
     (lambda () (test k))
     (lambda (result) 
       (display result)
