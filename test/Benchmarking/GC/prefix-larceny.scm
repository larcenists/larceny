(define-syntax if-fixflo
  (syntax-rules () 
    ((if-fixflo SPECIALIZED UNSPECIALIZED) UNSPECIALIZED)))

;INSERTCODE
;------------------------------------------------------------------------------

(define (dump-bmark-heap) (dump-interactive-heap "bmark.heap"))
(issue-warnings #f) ; Felix does not care to hear about macro redefinitions
(error-handler 
 (lambda l 
   (decode-error l)
